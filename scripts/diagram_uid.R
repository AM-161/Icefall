...
# Outputs:
#   data/ModelRuns/model_uidXXX.csv
#   site/plots/uid_XXX.png
# =====================================================================
...
# Settings (global)
# ----------------------------
# UIDs per CLI-Argumente (mehrere erlaubt) oder ENV
args <- commandArgs(trailingOnly = TRUE)

UIDS <- if (length(args) >= 1) {
  suppressWarnings(as.integer(args))
} else {
  suppressWarnings(as.integer(Sys.getenv("UID_TEST", "48")))
}

UIDS <- UIDS[is.finite(UIDS)]
if (length(UIDS) == 0) stop("Keine gültigen UIDs übergeben.")
...
# (Minimal-caching wie bisher – Dateiname enthält Datum; funktioniert nur innerhalb des Runs gut.
#  Wenn du inkrementell willst, sag kurz, dann baue ich dir die Append-Version rein.)
...
get_geosphere_station_tlrf <- function(start_date, end_date, station_id) {
  ...
  stop(
    "GeoSphere klima-v2-10min Station-Request failed. HTTP ", resp_status(resp), ". Response: ", last_msg
  )
}
...
# Station-Cache (spart, wenn viele UIDs dieselbe Station haben)
.station_cache <- new.env(parent = emptyenv())
get_station_tlrf_cached <- function(start_date, end_date, station_id, source) {
  key <- paste(source, station_id, as.Date(start_date), as.Date(end_date), sep = "|")
  if (exists(key, envir = .station_cache, inherits = FALSE)) {
    return(get(key, envir = .station_cache, inherits = FALSE))
  }
  x <- get_station_tlrf(start_date, end_date, station_id, source)
  assign(key, x, envir = .station_cache)
  x
}

# =====================================================================
# 1) Inputs laden (einmal!)
# =====================================================================
...
# 2) Pro UID bauen
# =====================================================================
build_one_uid <- function(uid_val) {
  
  # per-UID outputs
  PATH_OUT  <- sprintf("data/ModelRuns/model_uid%03d.csv", uid_val)
  plot_file <- sprintf("site/plots/uid_%03d.png", uid_val)
  
  row_uid <- assign %>% dplyr::filter(.data$uid == uid_val) %>% dplyr::slice(1)
  if (nrow(row_uid) == 0) stop("uid nicht gefunden: ", uid_val)
  ...
  st_meta <- stations_all %>% filter(as.character(.data$station_id) == station_id) %>% slice(1)
  z_aws <- if (nrow(st_meta) == 1) to_num(st_meta$altitude_m) else NA_real_
  invisible(z_aws) # (derzeit nicht genutzt)
  
  # =====================================================================
  # Sun + Wind LUT
  # =====================================================================
  sun_uid <- sun_all %>%
    filter(.data$uid == uid_val) %>%
    mutate(
      date = as.Date(.data$date),
      sunrise_topo = parse_time_any(.data$sunrise_topo, tz = TZ_LOCAL),
      sunset_topo  = parse_time_any(.data$sunset_topo,  tz = TZ_LOCAL),
      sun_hours_topo = to_num(.data$sun_hours_topo)
    ) %>%
    select(date, sunrise_topo, sunset_topo, sun_hours_topo) %>%
    distinct(date, .keep_all = TRUE) %>%
    filter(date >= START_DATE, date <= END_DATE_EXT)
  
  if (nrow(sun_uid) == 0) stop("Keine Sun-Daten für uid ", uid_val)
  
  wind_uid <- wind_lut %>%
    filter(.data$uid == uid_val) %>%
    mutate(dir_deg = as.numeric(.data$dir_deg), wind_vuln_0_9 = as.integer(.data$wind_vuln_0_9)) %>%
    select(dir_deg, wind_vuln_0_9)
  
  if (nrow(wind_uid) == 0) stop("Keine Wind-LUT für uid ", uid_val)
  
  # =====================================================================
  # Station TL/RF (10-min) -> 10-min Raster + Fill
  # =====================================================================
  wx10 <- get_station_tlrf_cached(START_DATE, END_DATE, station_id, source) %>%
    mutate(
      timestamp = with_tz(as.POSIXct(.data$timestamp, tz = TZ_LOCAL), TZ_LOCAL),
      TL = to_num(.data$TL),
      RF = to_num(.data$RF)
    )
  ...
  wx <- wx10 %>%
    mutate(time = floor_date(.data$timestamp, unit = step_str)) %>%
    group_by(.data$time) %>%
    summarise(
      TL = mean(.data$TL, na.rm = TRUE),
      RF = mean(.data$RF, na.rm = TRUE),
      .groups = "drop"
    ) %>%
    tidyr::complete(time = seq(min(.data$time), max(.data$time), by = step_str)) %>%
    arrange(.data$time) %>%
    mutate(
      TL = fill1(.data$TL),
      RF = fill1(.data$RF)
  ...
  wx <- wx %>%
    ...
    arrange(.data$time) %>%
    mutate(
      FF_inca   = zoo::na.locf(.data$FF_inca,   na.rm = FALSE),
      DD_inca   = zoo::na.locf(.data$DD_inca,   na.rm = FALSE),
      GLOW_inca = zoo::na.locf(.data$GLOW_inca, na.rm = FALSE),
      FF_inca   = zoo::na.locf(.data$FF_inca,   na.rm = FALSE, fromLast = TRUE),
      DD_inca   = zoo::na.locf(.data$DD_inca,   na.rm = FALSE, fromLast = TRUE),
      GLOW_inca = zoo::na.locf(.data$GLOW_inca, na.rm = FALSE, fromLast = TRUE),
      FF   = coalesce(.data$FF_inca, 0),
      DD   = coalesce(.data$DD_inca, 0),
      GLOW = coalesce(.data$GLOW_inca, 0)
    ) %>%
  ...
  # =====================================================================
  # NWP Forecast (60h) hourly -> 10-min LOCF
  # =====================================================================
  ...
  wx_fc <- tibble(time = seq(HIST_END + minutes(MODEL_STEP_MIN), FC_END, by = step_str)) %>%
    mutate(time_hr = floor_date(.data$time, "hour")) %>%
    left_join(nwp_hr %>% rename(time_hr = time), by = "time_hr") %>%
    select(-time_hr) %>%
    arrange(.data$time) %>%
    mutate(
      TL   = zoo::na.locf(.data$TL,   na.rm = FALSE),
      RF   = zoo::na.locf(.data$RF,   na.rm = FALSE),
      FF   = zoo::na.locf(.data$FF,   na.rm = FALSE),
      DD   = zoo::na.locf(.data$DD,   na.rm = FALSE),
      GLOW = zoo::na.locf(.data$GLOW, na.rm = FALSE),
      TL   = zoo::na.locf(.data$TL,   na.rm = FALSE, fromLast = TRUE),
      RF   = zoo::na.locf(.data$RF,   na.rm = FALSE, fromLast = TRUE),
      FF   = zoo::na.locf(.data$FF,   na.rm = FALSE, fromLast = TRUE),
      DD   = zoo::na.locf(.data$DD,   na.rm = FALSE, fromLast = TRUE),
      GLOW = zoo::na.locf(.data$GLOW, na.rm = FALSE, fromLast = TRUE)
    )
  ...
  wx <- wx %>%
    ...
    mutate(
      topo_sun_fac = ifelse(
        !is.na(.data$sunrise_topo) & !is.na(.data$sunset_topo) & .data$time >= .data$sunrise_topo & .data$time < .data$sunset_topo,
        1, 0
      ),
      dd_bin = bin5(.data$DD)
    ) %>%
    ...
    mutate(
      wind_vuln_0_9 = ifelse(is.na(.data$wind_vuln_0_9), 9L, .data$wind_vuln_0_9),
      wind_vuln = pmin(1, pmax(0, .data$wind_vuln_0_9 / 9))
    )
  ...
  wx <- wx %>%
    mutate(
      dz_eff = ifelse(.data$is_forecast, 0, dz_m),
      TLz    = .data$TL - coef$lapse_K_per_m * .data$dz_eff,
      FDH = pmax(0, -.data$TLz),
      PDH = pmax(0,  .data$TLz),
      
      GLOW = ifelse(is.finite(.data$GLOW), .data$GLOW, 0),
      SW_MJ_step = .data$GLOW * W2MJ_STEP * .data$topo_sun_fac * (1 - ice_params$albedo),
      
      FF_eff   = pmin(coef$wind_cap_ms, pmax(0, .data$FF)),
      wind_fac = 1 + coef$k_wind * .data$FF_eff * .data$wind_vuln,
      dry_fac  = 1 + coef$k_dry  * pmax(0, 1 - .data$RF/100),
      
      base_growth_mm_step = coef$growth_mm_per_C_h * .data$FDH * DT_H * .data$wind_fac * .data$dry_fac,
      base_melt_mm_step   = coef$melt_mm_per_C_h * .data$PDH * DT_H * .data$wind_fac +
        coef$rad_melt_mm_per_MJ * .data$SW_MJ_step
    )
  ...
  # =====================================================================
  # Climbability (0..1) + Hist daily smoothing
  # =====================================================================
  ...
  mod <- mod %>%
    mutate(
      TLz_72h = zoo::rollapplyr(
        .data$TLz, width = WIN_72H,
        FUN = function(x) mean(x, na.rm = TRUE),
        fill = NA_real_, partial = TRUE
      ),
      score_h  = pmin(1, pmax(0, (.data$thickness_m - H_MIN) / (H_OPT - H_MIN))),
      score_T  = score_T_fun_vec(.data$TLz,     T_OPT,  T_MIN,  T_MAX,  RANGE_T),
      score_T3 = score_T_fun_vec(.data$TLz_72h,  T3_OPT, T3_MIN, T3_MAX, RANGE_T3),
      score_RH = exp(-((.data$RF/100) - RH_OPT)^2 / (2 * RH_SIG^2)),
      climbability = .data$score_h * .data$score_T * .data$score_T3 * .data$score_RH,
      climbability = ifelse(.data$thickness_m < H_MIN, NA_real_, .data$climbability),
      climbability = pmin(1, pmax(0, .data$climbability)),
  ...
  climb_hist_daily <- mod %>%
    filter(!.data$is_forecast) %>%
    group_by(.data$date) %>%
    summarise(
      time = as.POSIXct(paste0(first(.data$date), " 12:00:00"), tz = TZ_LOCAL),
      climbability = mean_na(.data$climbability),
      .groups = "drop"
    )
  ...
  if (!has_fc) {
    ...
    mod <- mod %>% mutate(climb_y = y_min + .data$climbability * Y_DEN)
    climb_hist_daily <- climb_hist_daily %>% mutate(climb_y = y_min + .data$climbability * Y_DEN)
    ...
    if (nrow(mod_fc) > 0) {
      tmp <- mod_fc %>%
        ...
        mutate(
          is_sun = .data$topo_sun_fac == 1,
          run = cumsum(.data$is_sun != dplyr::lag(.data$is_sun, default = dplyr::first(.data$is_sun)))
        )
      
      tmp_sun <- tmp %>% filter(.data$is_sun)
      if (nrow(tmp_sun) > 0) {
        sun_rects_fc <- tmp_sun %>%
          group_by(.data$run) %>%
          reframe(
            xmin = min(.data$time),
            xmax = max(.data$time) + minutes(MODEL_STEP_MIN),
            ymin = -Inf,
            ymax =  Inf
          )
      }
    ...
    peak_fc <- mod_fc %>%
      filter(is.finite(.data$climbability)) %>%
      group_by(.data$date) %>%
      slice_max(order_by = .data$climbability, n = 1, with_ties = FALSE) %>%
      ungroup() %>%
      transmute(
        peak_time  = .data$time,
        peak_label = format(.data$time, "%d.%m\n%H:%M")
      )
...
for (uid_val in UIDS) {
  message("\n==============================")
  message("📈 Build Plot UID: ", uid_val)
  message("==============================")
  
  ok <- TRUE
  tryCatch(
    build_one_uid(uid_val),
    error = function(e) {
      ok <<- FALSE
      message("❌ UID ", uid_val, " fehlgeschlagen: ", e$message)
    }
  )
  
  if (!ok) failed <- c(failed, uid_val)
}

if (length(failed) > 0) {
  stop("Fehlgeschlagene UIDs: ", paste(failed, collapse = ", "))
}
