...
# =============================================================
# PATCH: Weniger verpixelt + Fix für leaflet 2.2.3 (method fehlt)
# =============================================================
#Upscaling

do_upsample <- TRUE   # FALSE, wenn es zu langsam / zu groß wird
fine_factor <- 4      # 2 = moderat, 4 = deutlich glatter

r_template_fine <- raster::raster(
  raster::extent(r_template),
  crs = raster::crs(r_template),
  res = raster::res(r_template) / fine_factor
)
...
if (length(ice_time_layers) > 0L) {
  ...
  for (i in seq_along(ice_time_layers)) {
    ...
    if (!is.null(r_i)) {
      ...
      r_plot <- r_i
      if (isTRUE(do_upsample)) {
        r_plot <- raster::resample(r_i, r_template_fine, method = 'bilinear')
      }
      ...
      m <- m |>
        addRasterImage(
          ...
          project = TRUE,
          method  = 'bilinear',   # <- WICHTIG (leaflet 2.2.3)
          group   = 'Eisdicke',
