...
# - SPEED PATCH für GitHub Pages:
#   (1) KEIN selfcontained-HTML (kleine index.html)
#   (2) Raster-Zeitstufen NICHT als Base64 in HTML einbetten
#       -> als externe PNG-Overlays ausgeben (site/img/*.png)
#       -> Leaflet hat nur 2 Overlays (Eisdicke + Climbability) und
#          der Slider wechselt nur die URL (setUrl)
#   (3) Deploy nur den Ordner site/ (nicht das ganze Repo)
# =====================================================================
...
bounds <- list(lng1 = ext@xmin, lat1 = ext@ymin, lng2 = ext@xmax, lat2 = ext@ymax)
...
# **WICHTIG**: externe Overlays, keine Base64-Raster!

m <- m |>
  addImageOverlay(
    url     = sprintf("img/ice_%03d.png", init_i),
    bounds  = bounds,
    opacity = 0.8,
    group   = "Eisdicke",
    layerId = "ice_overlay"
  ...
  addImageOverlay(
    url     = sprintf("img/climb_%03d.png", init_i),
    bounds  = bounds,
    opacity = 0.7,
    group   = "Climbability",
    layerId = "climb_overlay"
  )
