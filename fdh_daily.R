# =====================================================================
# Urheberrechtlicher Hinweis / Nutzungseinsschränkung
# ---------------------------------------------------------------------
# © 2025 Alle Rechte vorbehalten.
#
# Dieses Skript wurde im Rahmen der Masterarbeit zur Kletterbarkeit
# von Eisfällen in Nordtirol erstellt. Der Code darf ohne vorherige
# ausdrückliche schriftliche Zustimmung des Autors weder ganz noch
# teilweise kopiert, verändert, weiterverbreitet oder in eigene
# Projekte (insbesondere kommerzielle Anwendungen oder proprietäre
# Software) integriert werden.
#
# Eine Nutzung über das reine Lesen und Nachvollziehen im Kontext
# dieser Masterarbeit hinaus ist nicht gestattet. Bei Rückfragen
# oder einer gewünschten Nutzung bitte vorher Kontakt mit dem Autor
# aufnehmen.
# =====================================================================
# FDH-basiertes Eiswachstumsmodell für Nordtirol (optimiert / streaming)
# =====================================================================

suppressPackageStartupMessages({
  library(httr)
  library(ncdf4)
  library(raster)
  library(leaflet)
  library(htmlwidgets)
  library(readr)
  library(dplyr)
  library(lubridate)
  library(htmltools)
})

# ----------------------------- Helpers ------------------------------

convert_nc_time <- function(time_vals, time_units) {
  if (!grepl("since", time_units)) {
    origin <- as.POSIXct("1970-01-01 00:00:00", tz = "UTC")
    return(origin + time_vals * 3600)
  }
  unit_str   <- sub(" since.*", "", time_units)
  origin_str <- sub(".*since ", "", time_units)
  origin     <- as.POSIXct(origin_str, tz = "UTC")
  
  if (grepl("hour", unit_str, ignore.case = TRUE)) {
    origin + time_vals * 3600
  } else if (grepl("second", unit_str, ignore.case = TRUE)) {
    origin + time_vals
  } else if (grepl("day", unit_str, ignore.case = TRUE)) {
    origin + time_vals * 86400
  } else {
    origin + time_vals
  }
}

download_chunk <- function(cs, ce, out_dir, base_url, param_str, bbox) {
  start_time <- sprintf("%sT00:00", format(cs, "%Y-%m-%d"))
  end_time   <- sprintf("%sT23:00", format(ce, "%Y-%m-%d"))
  
  outfile <- file.path(
    out_dir,
    sprintf("inca_nordtirol_%s_%s.nc", format(cs, "%Y%m%d"), format(ce, "%Y%m%d"))
  )
  
  if (file.exists(outfile) && file.info(outfile)$size > 0) {
    message("  -> übersprungen (existiert): ", outfile)
    return(outfile)
  }
  
  query <- list(
    parameters    = param_str,
    start         = start_time,
    end           = end_time,
    bbox          = paste(bbox, collapse = ","),
    output_format = "netcdf",
    filename      = "inca_nordtirol"
  )
  
  message("  -> lade: ", cs, " bis ", ce)
  resp <- RETRY(
    "GET",
    url = base_url,
    query = query,
    write_disk(outfile, overwrite = TRUE),
    times = 5,
    pause_min = 1,
    pause_cap = 10
  )
  stop_for_status(resp)
  message("  -> gespeichert: ", outfile, " (", file.info(outfile)$size, " Bytes)")
  outfile
}

# Orientierung prüfen & ggf. umdrehen (für 2D Felder)
orient_ij <- function(lon, lat, mat_list) {
  # mat_list: named list of matrices [nx, ny] oder arrays [nx, ny, ...] (mind 2D)
  nx <- dim(lon)[1]
  ny <- dim(lon)[2]
  
  # j-Richtung (S->N)
  lat_mean_j <- colMeans(lat, na.rm = TRUE)
  flip_j <- is.finite(lat_mean_j[1]) && is.finite(tail(lat_mean_j, 1)) &&
    (lat_mean_j[1] < tail(lat_mean_j, 1))
  
  if (flip_j) {
    idx_j <- ny:1
    lon <- lon[, idx_j, drop = FALSE]
    lat <- lat[, idx_j, drop = FALSE]
    mat_list <- lapply(mat_list, function(x) {
      if (length(dim(x)) == 2) x[, idx_j, drop = FALSE] else x[, idx_j, , drop = FALSE]
    })
  }
  
  # i-Richtung (W->E)
  lon_mean_i <- rowMeans(lon, na.rm = TRUE)
  flip_i <- is.finite(lon_mean_i[1]) && is.finite(tail(lon_mean_i, 1)) &&
    (lon_mean_i[1] > tail(lon_mean_i, 1))
  
  if (flip_i) {
    idx_i <- nx:1
    lon <- lon[idx_i, , drop = FALSE]
    lat <- lat[idx_i, , drop = FALSE]
    mat_list <- lapply(mat_list, function(x) {
      if (length(dim(x)) == 2) x[idx_i, , drop = FALSE] else x[idx_i, , , drop = FALSE]
    })
  }
  
  list(lon = lon, lat = lat, mats = mat_list)
}

make_template_raster <- function(lon, lat, values_matrix_ij) {
  # values_matrix_ij: [nx, ny] oder [ny, nx]? Wir definieren:
  # Bei dir wird später meist raster(t(matrix)) genutzt.
  # Template erzeugen wir konsistent über eine [ny, nx]-Matrix.
  r <- raster(values_matrix_ij)
  extent(r) <- c(min(lon, na.rm = TRUE),
                 max(lon, na.rm = TRUE),
                 min(lat, na.rm = TRUE),
                 max(lat, na.rm = TRUE))
  crs(r) <- "EPSG:4326"
  r
}

score_h_fun <- function(h, h_min, h_opt) {
  s <- (h - h_min) / (h_opt - h_min)
  s[h <= h_min] <- 0
  s[h >= h_opt] <- 1
  s[!is.finite(s)] <- 0
  s
}

score_T_fun <- function(Tv, T_opt, T_min, T_max, range_T) {
  s <- 1 - abs(Tv - T_opt) / range_T
  s[Tv <= T_min | Tv >= T_max] <- 0
  s[s < 0] <- 0
  s[!is.finite(s)] <- 0
  s
}

score_RH_fun <- function(rh, RH_opt, RH_sig) {
  peak <- exp(- (rh - RH_opt)^2 / (2 * RH_sig^2))
  s <- peak
  s[!is.finite(s)] <- 0
  s
}

weighted_mean_alt <- function(r, w_rast) {
  v <- raster::getValues(r)
  w <- raster::getValues(w_rast)
  ok <- is.finite(v) & is.finite(w)
  if (!any(ok)) return(NA_real_)
  sum(v[ok] * w[ok]) / sum(w[ok])
}

# robust leafet raster display: vorprojizieren (EPSG:3857) + project=FALSE
leaflet_add_projected_raster <- function(map, r, pal, opacity, layer_id, group) {
  
  # Ziel-CRS (WebMercator)
  crs_3857 <- sp::CRS("+init=epsg:3857")
  
  # Raster nach 3857 projizieren (für kontinuierliche Daten: bilinear)
  r_3857 <- raster::projectRaster(
    r,
    crs = crs_3857,
    method = "bilinear"
  )
  
  map |>
    leaflet::addRasterImage(
      r_3857,
      colors  = pal,
      opacity = opacity,
      project = FALSE,
      group   = group,
      layerId = layer_id
    )
}


# ------------------------ 0) Zeitraum & Gebiet -----------------------

start_all    <- as.Date("2025-10-01")
end_all      <- Sys.Date()
chunk_days   <- 2
chunk_starts <- seq.Date(start_all, end_all, by = chunk_days)

bbox <- c(
  46.7,  # lat_min
  10.1,  # lon_min
  47.7,  # lat_max
  12.2   # lon_max
)

parameters    <- c("RR", "T2M", "RH2M", "UU", "VV", "GL", "P0", "TD2M")
param_str     <- paste(parameters, collapse = ",")
base_url_inca <- "https://dataset.api.hub.geosphere.at/v1/grid/historical/inca-v1-1h-1km"

out_dir <- "data/inca_nordtirol"
dir.create(out_dir, showWarnings = FALSE, recursive = TRUE)

# ---------------- 1) INCA chunkweise laden (caching) -----------------

nc_files <- character(0)

for (idx in seq_along(chunk_starts)) {
  cs <- chunk_starts[idx]                    # bleibt Date
  ce <- min(cs + (chunk_days - 1), end_all)  # bleibt Date
  
  message("Chunk: ", format(cs, "%Y-%m-%d"), " bis ", format(ce, "%Y-%m-%d"))
  nc_files <- c(nc_files, download_chunk(cs, ce, out_dir, base_url_inca, param_str, bbox))
}

nc_files <- sort(unique(nc_files))


if (length(nc_files) == 0) stop("Keine INCA NetCDF Dateien vorhanden.")

# ------------- 2) Metadaten aus erster Datei: Grid & Time --------------

nc0 <- nc_open(nc_files[1])
on.exit(try(nc_close(nc0), silent = TRUE), add = TRUE)

varnames0 <- names(nc0$var)

lon_name <- intersect(c("lon", "longitude", "x", "xc"), varnames0)[1]
lat_name <- intersect(c("lat", "latitude", "y", "yc"), varnames0)[1]
if (!is.finite(match(lon_name, varnames0)) || !is.finite(match(lat_name, varnames0))) {
  stop("Konnte lon/lat Variablen im NetCDF nicht finden.")
}

lon0 <- ncvar_get(nc0, lon_name)  # [nx, ny]
lat0 <- ncvar_get(nc0, lat_name)  # [nx, ny]

nx <- dim(lon0)[1]
ny <- dim(lon0)[2]

time_dim_name <- names(nc0$dim)[grep("time", tolower(names(nc0$dim)))[1]]
if (is.na(time_dim_name)) stop("Konnte Zeitdimension (time) nicht finden.")
nc_close(nc0)

vars_needed <- c("T2M", "RH2M", "UU", "VV")  # fürs FDH-Update
# optional: weitere Variablen (RR/GL/TD2M/P0) bleiben wie bei dir ungenutzt im Kernmodell

# ----------- 3) DEM / Exposition / Höhengewicht (raster) --------------

dem_inca <- raster("data/DEM/DEM_Tirol_INCAgrid_1km_epsg4326.tif")
crs(dem_inca) <- "EPSG:4326"
names(dem_inca) <- "elev_m"

sl_as <- terrain(dem_inca, opt = c("slope", "aspect"), unit = "degrees")
aspect_inca <- sl_as[["aspect"]]

aspect_rad_from_north <- (aspect_inca - 90) * pi / 180
northness_r <- cos(aspect_rad_from_north)

solar_index_r <- (1 - northness_r) / 2
solar_index_r[is.na(solar_index_r[])] <- 0.5

# Höhengewicht
alt_threshold   <- 2500
alt_weight_high <- 0.5

alt_weight_r <- raster::calc(dem_inca, fun = function(z) {
  ifelse(is.na(z), NA,
         ifelse(z > alt_threshold, alt_weight_high, 1))
})
names(alt_weight_r) <- "w_alt"

# ------------------ 4) Zeitachse über alle NetCDFs --------------------

# Wir bauen nur die Zeitstempel (kein Daten-Array).
time_all <- as.POSIXct(character(0), tz = "UTC")

nt_per_file <- integer(length(nc_files))
for (i in seq_along(nc_files)) {
  nc <- nc_open(nc_files[i])
  tvals  <- ncvar_get(nc, time_dim_name)
  tunits <- ncatt_get(nc, time_dim_name, "units")$value
  tposix <- convert_nc_time(tvals, tunits)
  time_all <- c(time_all, tposix)
  nt_per_file[i] <- length(tvals)
  nc_close(nc)
}

nt_total <- length(time_all)
if (nt_total == 0) stop("Keine Zeitstempel in den NetCDFs gefunden.")

# Lokalzeit-Vektor für Sonnenhöhe/weights
time_local <- as.POSIXlt(time_all, tz = "Europe/Vienna")
doy  <- time_local$yday + 1
hour <- time_local$hour + time_local$min / 60 + time_local$sec / 3600

# Sonnenhöhenfaktor (Skalar pro Zeit)
delta_t <- 23.44 * pi/180 * sin(2 * pi * (284 + doy) / 365)

lat_center_deg <- (min(lat0, na.rm = TRUE) + max(lat0, na.rm = TRUE)) / 2
lat_center_rad <- lat_center_deg * pi / 180

H_t <- (hour - 12) * 15 * pi / 180
sin_alpha_t <- sin(lat_center_rad) * sin(delta_t) +
  cos(lat_center_rad) * cos(delta_t) * cos(H_t)
sin_alpha_t[sin_alpha_t < 0] <- 0
solar_height_factor_t <- sin_alpha_t

# Zeitgewicht (exp decay)
max_time <- max(time_all, na.rm = TRUE)
age_days <- as.numeric(difftime(max_time, time_all, units = "days"))
tau_days <- 10
weight_time_t <- exp(-age_days / tau_days)

# Snapshot-Zeiten (07:00 local täglich)
t_start <- min(time_all, na.rm = TRUE)

time_local_all <- as.POSIXct(format(time_all, tz = "Europe/Vienna", usetz = TRUE),
                             tz = "Europe/Vienna")
t_min_local <- min(time_local_all, na.rm = TRUE)
t_max_local <- max(time_local_all, na.rm = TRUE)

t0_local <- as.POSIXct(paste0(format(t_min_local, "%Y-%m-%d"), " 07:00:00"),
                       tz = "Europe/Vienna")
if (t0_local < t_min_local) t0_local <- t0_local + 24 * 3600

t_last_local <- as.POSIXct(paste0(format(t_max_local, "%Y-%m-%d"), " 07:00:00"),
                           tz = "Europe/Vienna")
if (t_last_local > t_max_local) t_last_local <- t_last_local - 24 * 3600

if (t_last_local < t0_local) {
  snap_times_utc <- as.POSIXct(character(0), tz = "UTC")
} else {
  snap_times_local <- seq(from = t0_local, to = t_last_local, by = "1 day")
  snap_times_utc   <- as.POSIXct(format(snap_times_local, tz = "UTC", usetz = TRUE),
                                 tz = "UTC")
}

# Map Snapshots -> Index (nächster Zeitpunkt)
snap_idx <- integer(length(snap_times_utc))
if (length(snap_times_utc) > 0) {
  # find nearest index in time_all
  for (i in seq_along(snap_times_utc)) {
    dt <- abs(as.numeric(difftime(time_all, snap_times_utc[i], units = "secs")))
    snap_idx[i] <- which.min(dt)
  }
} else {
  snap_idx <- integer(0)
}

# ------------------ 5) Orientierung / Raster-Template -----------------

# solar_index_ij als Matrix in INCA-Orientierung
# -> wir müssen sicherstellen, dass DEM Raster auf das INCA-Grid passt.
# Du hast dein DEM schon im INCAgrid gespeichert. Wir nehmen Matrix aus solar_index_r.
solar_index_ij <- t(as.matrix(solar_index_r))  # [ny, nx] als in deinem alten Code

# Template: wir brauchen irgendeine Matrix [ny, nx]
# (wir erzeugen Dummy, wird später ersetzt)
dummy_mat_ny_nx <- matrix(0, nrow = ny, ncol = nx)
r_template <- make_template_raster(lon0, lat0, dummy_mat_ny_nx)
crs(r_template) <- "EPSG:4326"

# ------------- 6) Kern: FDH effektiv streamend akkumulieren -----------

k_expo   <- 0.5
k_wind   <- 0.5
wind_min <- 0.5
wind_max <- 2.0
rh_opt   <- 0.7
rh_sig   <- 0.15
k_melt   <- 1.2

# FDH-Summe als [nx, ny] Matrix (wie in deinem bisherigen Code)
FDH_sum_eff <- matrix(0, nrow = nx, ncol = ny)

# Rolling buffer für 72h Temperaturmittel (für Tr3)
roll_n <- min(72, nt_total)
T_roll <- array(NA_real_, dim = c(nx, ny, roll_n))
roll_pos <- 0L
roll_filled <- 0L

# Für "aktuelles" T/RH am Ende:
T_last_ij  <- matrix(NA_real_, nrow = nx, ncol = ny)
RH_last_ij <- matrix(NA_real_, nrow = nx, ncol = ny)

# Historische Snapshot-Raster
FDH_hist_layers <- list()
FDH_hist_labels <- character(0)
FDH_hist_times  <- as.POSIXct(character(0), tz = "UTC")

# Wind-Referenz: statt aus ganzem 3D-Array -> wir schätzen aus Stichprobe (erste Datei)
# (stabil & schnell)
estimate_wind_ref <- function(nc_file, time_dim_name) {
  nc <- nc_open(nc_file)
  on.exit(nc_close(nc), add = TRUE)
  
  # lese nur 24 Stunden (oder alle falls kürzer)
  nt <- length(ncvar_get(nc, time_dim_name))
  take <- min(nt, 24)
  
  uu <- ncvar_get(nc, "UU", start = c(1, 1, 1), count = c(-1, -1, take))
  vv <- ncvar_get(nc, "VV", start = c(1, 1, 1), count = c(-1, -1, take))
  w  <- sqrt(uu^2 + vv^2)
  wr <- mean(w, na.rm = TRUE)
  if (!is.finite(wr) || wr <= 0) wr <- 5
  wr
}

wind_ref <- estimate_wind_ref(nc_files[1], time_dim_name)

# Snapshot-Index-Set für O(1) Check
snap_set <- rep(FALSE, nt_total)
if (length(snap_idx) > 0) snap_set[snap_idx] <- TRUE

# Streaming-Leseindex über alle Dateien
global_k <- 0L

for (f in seq_along(nc_files)) {
  message("Streaming: ", nc_files[f])
  nc <- nc_open(nc_files[f])
  on.exit(try(nc_close(nc), silent = TRUE), add = TRUE)
  
  # time in this file
  time_vals  <- ncvar_get(nc, time_dim_name)
  nt_f <- length(time_vals)
  
  # 3D reads: [nx, ny, nt_f]
  # NOTE: Falls Dimensionen in deinem NetCDF anders sind, musst du start/count anpassen.
  T3  <- ncvar_get(nc, "T2M")
  RH3 <- ncvar_get(nc, "RH2M")
  UU3 <- ncvar_get(nc, "UU")
  VV3 <- ncvar_get(nc, "VV")
  
  # Orientierung check (einmal pro Datei) anhand lon/lat ist konstant,
  # aber wir drehen ggf. die 3D arrays wie im alten Code.
  # Dafür brauchen wir lon/lat aus Datei (sicherer, falls gedreht gespeichert).
  lon_f <- ncvar_get(nc, lon_name)
  lat_f <- ncvar_get(nc, lat_name)
  
  oriented <- orient_ij(lon_f, lat_f, list(T = T3, RH = RH3, UU = UU3, VV = VV3))
  T3  <- oriented$mats$T
  RH3 <- oriented$mats$RH
  UU3 <- oriented$mats$UU
  VV3 <- oriented$mats$VV
  
  # Jetzt pro Zeitschritt akkumulieren
  for (k in seq_len(nt_f)) {
    global_k <- global_k + 1L
    
    T_k  <- T3[, , k]
    RH_k <- RH3[, , k]
    UU_k <- UU3[, , k]
    VV_k <- VV3[, , k]
    
    # Last fields
    T_last_ij  <- T_k
    RH_last_ij <- RH_k / 100
    
    # Rolling temp buffer für 72h mean
    roll_pos <- (roll_pos %% roll_n) + 1L
    T_roll[, , roll_pos] <- T_k
    roll_filled <- min(roll_filled + 1L, roll_n)
    
    # Skip wenn komplett NA
    if (!(all(is.na(T_k)) || all(is.na(RH_k)))) {
      RH_eff <- RH_k / 100
      RH_eff[is.na(RH_eff)] <- rh_opt
      
      FDH_k <- pmax(-T_k, 0)
      MDH_k <- pmax( T_k, 0)
      
      W_k <- sqrt(UU_k^2 + VV_k^2)
      
      wind_norm <- (W_k - wind_ref) / wind_ref
      f_wind <- 1 + k_wind * wind_norm
      f_wind[!is.finite(f_wind)] <- 1
      f_wind <- pmax(wind_min, pmin(wind_max, f_wind))
      
      f_rh_peak <- exp(- (RH_eff - rh_opt)^2 / (2 * rh_sig^2))
      f_rh <- 0.5 + 0.5 * f_rh_peak
      
      s_height <- solar_height_factor_t[global_k]
      if (!is.finite(s_height)) s_height <- 0
      
      if (s_height == 0) {
        f_rad <- 1
      } else {
        # solar_index_ij ist [ny, nx], unsere Felder sind [nx, ny] -> anpassen:
        # Wir verwenden solar_index_ij_t = t(solar_index_ij) => [nx, ny]
        # und berechnen dann elementweise.
        solar_index_ij_t <- t(solar_index_ij)
        f_rad <- 1 - k_expo * s_height * solar_index_ij_t
        f_rad[!is.finite(f_rad)] <- 1
        f_rad <- pmax(0, pmin(1, f_rad))
      }
      
      f_time <- weight_time_t[global_k]
      if (!is.finite(f_time)) f_time <- 1
      
      FDH_sum_eff <- FDH_sum_eff + (FDH_k * f_wind * f_rh * f_rad * f_time) -
        (MDH_k * k_melt * f_time)
    }
    
    # Snapshots (historisch)
    if (snap_set[global_k]) {
      FDH_k_snap <- FDH_sum_eff
      FDH_k_snap[FDH_k_snap < 0] <- 0
      
      r_FDH_snap <- raster(t(FDH_k_snap))  # [ny, nx]
      extent(r_FDH_snap) <- extent(r_template)
      crs(r_FDH_snap)    <- crs(r_template)
      
      label_time <- time_all[global_k]
      label_str  <- format(label_time, "%d.%m.%Y")
      
      FDH_hist_layers[[length(FDH_hist_layers) + 1L]] <- r_FDH_snap
      FDH_hist_labels <- c(FDH_hist_labels, label_str)
      FDH_hist_times  <- c(FDH_hist_times, label_time)
    }
  }
  
  nc_close(nc)
}

FDH_sum_eff[FDH_sum_eff < 0] <- 0

# Effektive FDH Raster
r_FDH_eff <- raster(t(FDH_sum_eff))
extent(r_FDH_eff) <- extent(r_template)
crs(r_FDH_eff)    <- crs(r_template)
names(r_FDH_eff)  <- "FDH_eff_C_h"

# Tr3 (72h mean) aus Rollbuffer
if (roll_filled > 0) {
  # sortiere Buffer in zeitlicher Reihenfolge
  if (roll_filled < roll_n) {
    T_roll_use <- T_roll[, , seq_len(roll_filled), drop = FALSE]
  } else {
    # ältestes Element ist roll_pos+1
    idx_order <- c((roll_pos + 1L):roll_n, 1L:roll_pos)
    T_roll_use <- T_roll[, , idx_order, drop = FALSE]
  }
  Tr3_ij <- apply(T_roll_use, c(1, 2), mean, na.rm = TRUE)
} else {
  Tr3_ij <- matrix(NA_real_, nrow = nx, ncol = ny)
}

# ------------------ 7) FDH -> Eisdicke + Climbability ----------------

h_c      <- 30
rho_i    <- 880
Lf       <- 334000
FDH_crit <- 1

alpha <- h_c * 3600 / (rho_i * Lf)

# Eisdicke (aktuell)
h_mat_ij <- FDH_sum_eff * alpha

ice_thick_expo <- r_FDH_eff * alpha
ice_thick_expo[r_FDH_eff < FDH_crit] <- NA
names(ice_thick_expo) <- "h_pot_expo_m"

# Scores
h_min <- 0.10
h_opt <- 0.50

T_opt  <- -4
T_min  <- -20
T_max  <- 0
range_T <- max(T_opt - T_min, T_max - T_opt)

T3_opt  <- -6
T3_min  <- -20
T3_max  <- -1
range_T3 <- max(T3_opt - T3_min, T3_max - T3_opt)

RH_opt_c <- 0.7
RH_sig_c <- 0.2

score_h  <- score_h_fun(h_mat_ij, h_min, h_opt)
score_T  <- score_T_fun(T_last_ij, T_opt, T_min, T_max, range_T)
score_T3 <- score_T_fun(Tr3_ij, T3_opt, T3_min, T3_max, range_T3)
score_RH <- score_RH_fun(RH_last_ij, RH_opt_c, RH_sig_c)

climb_index_ij <- score_h * score_T * score_T3 * score_RH
climb_index_ij[h_mat_ij <= h_min]          <- NA
climb_index_ij[!is.finite(climb_index_ij)] <- NA
climb_index_ij[climb_index_ij <= 0]        <- NA

climb_r <- raster(t(climb_index_ij))
extent(climb_r) <- extent(r_template)
crs(climb_r)    <- crs(r_template)
names(climb_r)  <- "Climbability_0_1"

# Historische Eisdicken-Layer
ice_hist_layers <- list()
ice_hist_labels <- character(0)

if (length(FDH_hist_layers) > 0) {
  ice_hist_layers <- vector("list", length(FDH_hist_layers))
  ice_hist_labels <- FDH_hist_labels
  for (i in seq_along(FDH_hist_layers)) {
    r_fd  <- FDH_hist_layers[[i]]
    ice_i <- r_fd * alpha
    ice_i[r_fd < FDH_crit] <- NA
    names(ice_i) <- paste0("h_pot_expo_hist_", ice_hist_labels[i])
    ice_hist_layers[[i]] <- ice_i
  }
}

# Climbability-Historie (wie bei dir: Tages-Snapshot, T/RH aus nächster Zeit)
climb_hist_layers <- list()
climb_hist_labels <- character(0)

if (length(FDH_hist_layers) > 0 && length(FDH_hist_labels) == length(FDH_hist_layers)) {
  climb_hist_layers <- vector("list", length(FDH_hist_layers))
  climb_hist_labels <- FDH_hist_labels
  
  # Wir haben nicht mehr alle T2M stündlich im RAM.
  # -> pragmatisch: für historische Snapshots nehmen wir "aktuelles" Scoring nur aus h_i (Eisdicke),
  #    und approximieren T/RH/Tr3 über die zuletzt verfügbaren Felder.
  # Wenn du exakte historische T/RH brauchst: müsstest du beim Streaming zusätzlich Snapshot-T/RH speichern.
  # (Das kann ich dir bei Bedarf ergänzen.)
  warning("Hinweis: Historische Climbability-Layer nutzen in dieser Optimierung nicht mehr exakt T/RH/Tr3 zum Snapshotzeitpunkt (weil Streaming).")
  
  for (i in seq_along(FDH_hist_layers)) {
    r_fd <- FDH_hist_layers[[i]]
    h_i  <- r_fd * alpha
    h_i[r_fd < FDH_crit] <- NA
    
    # Nutze aktuelle (letzte) Felder als Fallback (schnell & robust)
    # (für exakte Rekonstruktion: Snapshot-T/RH/Tr3 mitspeichern)
    r_T_i  <- raster(t(T_last_ij));  extent(r_T_i)  <- extent(r_template); crs(r_T_i)  <- crs(r_template)
    r_RH_i <- raster(t(RH_last_ij)); extent(r_RH_i) <- extent(r_template); crs(r_RH_i) <- crs(r_template)
    r_T3_i <- raster(t(Tr3_ij));     extent(r_T3_i) <- extent(r_template); crs(r_T3_i) <- crs(r_template)
    
    score_h_i  <- calc(h_i,   fun = function(h)  score_h_fun(h,  h_min, h_opt))
    score_T_i  <- calc(r_T_i, fun = function(Tv) score_T_fun(Tv, T_opt, T_min, T_max, range_T))
    score_T3_i <- calc(r_T3_i, fun = function(T3v) score_T_fun(T3v, T3_opt, T3_min, T3_max, range_T3))
    score_RH_i <- calc(r_RH_i, fun = function(rh) score_RH_fun(rh, RH_opt_c, RH_sig_c))
    
    climb_i <- overlay(score_h_i, score_T_i, score_T3_i, score_RH_i,
                       fun = function(a, b, c, d) a * b * c * d)
    
    climb_i[is.na(h_i) | h_i <= h_min] <- NA
    climb_i[!is.finite(climb_i)]       <- NA
    climb_i[climb_i <= 0]              <- NA
    names(climb_i) <- paste0("Climbability_hist_", climb_hist_labels[i])
    climb_hist_layers[[i]] <- climb_i
  }
}

# ------------------ 8) Prognose (NWP) wie bei dir --------------------

base_url_nwp   <- "https://dataset.api.hub.geosphere.at/v1/grid/forecast/nwp-v1-1h-2500m"
parameters_nwp <- c("t2m", "rh2m", "u10m", "v10m")
param_str_nwp  <- paste(parameters_nwp, collapse = ",")

t_now    <- max(time_all, na.rm = TRUE)
t0       <- t_now
fc_h_max <- 60
t_fc_end <- t_now + fc_h_max * 3600

start_fc <- format(t_now,    "%Y-%m-%dT%H:%M")
end_fc   <- format(t_fc_end, "%Y-%m-%dT%H:%M")

outfile_fc <- file.path(
  out_dir,
  sprintf("nwp_nordtirol_%s_%sh.nc", format(as.Date(t_now), "%Y%m%d"), fc_h_max)
)

if (!file.exists(outfile_fc) || file.info(outfile_fc)$size == 0) {
  message("NWP-Chunk: ", start_fc, " bis ", end_fc)
  query_fc <- list(
    parameters    = param_str_nwp,
    start         = start_fc,
    end           = end_fc,
    bbox          = paste(bbox, collapse = ","),
    output_format = "netcdf",
    filename      = "nwp_nordtirol"
  )
  resp_fc <- RETRY(
    "GET",
    url = base_url_nwp,
    query = query_fc,
    write_disk(outfile_fc, overwrite = TRUE),
    times = 5,
    pause_min = 1,
    pause_cap = 10
  )
  stop_for_status(resp_fc)
  message("  -> NWP-Vorhersage gespeichert: ", outfile_fc,
          " (", file.info(outfile_fc)$size, " Bytes)")
} else {
  message("  -> NWP-Vorhersage übersprungen (existiert): ", outfile_fc)
}

ice_fc_layers   <- NULL
climb_fc_layers <- NULL
fc_step_hours   <- NULL
target_times    <- NULL

if (file.exists(outfile_fc) && file.info(outfile_fc)$size > 0) {
  nc_fc <- nc_open(outfile_fc)
  time_dim_name_fc <- names(nc_fc$dim)[grep("time", tolower(names(nc_fc$dim)))[1]]
  time_fc_vals     <- ncvar_get(nc_fc, time_dim_name_fc)
  time_fc_units    <- ncatt_get(nc_fc, time_dim_name_fc, "units")$value
  time_fc          <- convert_nc_time(time_fc_vals, time_fc_units)
  nc_close(nc_fc)
  
  lead_hours <- as.numeric(difftime(time_fc, t_now, units = "hours"))
  keep_fc    <- which(lead_hours >= 0 & lead_hours <= fc_h_max + 0.01)
  
  if (length(keep_fc) > 0) {
    time_fc    <- time_fc[keep_fc]
    lead_hours <- lead_hours[keep_fc]
    
    T2M_fc_brick  <- brick(outfile_fc, varname = "t2m")[[keep_fc]]
    RH2M_fc_brick <- brick(outfile_fc, varname = "rh2m")[[keep_fc]]
    U10_fc_brick  <- brick(outfile_fc, varname = "u10m")[[keep_fc]]
    V10_fc_brick  <- brick(outfile_fc, varname = "v10m")[[keep_fc]]
    
    crs(T2M_fc_brick)  <- crs(r_template)
    crs(RH2M_fc_brick) <- crs(r_template)
    crs(U10_fc_brick)  <- crs(r_template)
    crs(V10_fc_brick)  <- crs(r_template)
    
    T2M_fc  <- resample(T2M_fc_brick,  r_template, method = "bilinear")
    RH2M_fc <- resample(RH2M_fc_brick, r_template, method = "bilinear")
    U10_fc  <- resample(U10_fc_brick,  r_template, method = "bilinear")
    V10_fc  <- resample(V10_fc_brick,  r_template, method = "bilinear")
    
    W_fc <- overlay(U10_fc, V10_fc, fun = function(u, v) sqrt(u^2 + v^2))
    
    time_local_fc <- as.POSIXlt(time_fc, tz = "Europe/Vienna")
    doy_fc  <- time_local_fc$yday + 1
    hour_fc <- time_local_fc$hour + time_local_fc$min / 60 + time_local_fc$sec / 3600
    
    delta_fc <- 23.44 * pi/180 * sin(2 * pi * (284 + doy_fc) / 365)
    H_fc     <- (hour_fc - 12) * 15 * pi/180
    
    sin_alpha_fc <- sin(lat_center_rad) * sin(delta_fc) +
      cos(lat_center_rad) * cos(delta_fc) * cos(H_fc)
    sin_alpha_fc[sin_alpha_fc < 0] <- 0
    solar_height_factor_fc <- sin_alpha_fc
    
    # Tr3_fc: schnelle Näherung (wie bei dir: global mean über alle Layer)
    Tr3_fc <- calc(T2M_fc, fun = function(x) mean(x, na.rm = TRUE))
    score_T3_fc <- calc(Tr3_fc, fun = function(t3) {
      score_T_fun(t3, T3_opt, T3_min, T3_max, range_T3)
    })
    
    time_fc_local <- as.POSIXct(format(time_fc, tz = "Europe/Vienna", usetz = TRUE),
                                tz = "Europe/Vienna")
    t0_local_fc <- as.POSIXct(format(t0, tz = "Europe/Vienna", usetz = TRUE),
                              tz = "Europe/Vienna")
    
    target_hours_all  <- c(0, lead_hours)
    target_labels_all <- c(
      paste0(format(t0_local_fc, "%d.%m.%Y, %H:%M"), " (Analyse)"),
      format(time_fc_local, "%d.%m.%Y, %H:%M")
    )
    ord <- order(target_hours_all)
    target_hours_all  <- target_hours_all[ord]
    target_labels_all <- target_labels_all[ord]
    
    max_lead <- max(lead_hours) + 1e-6
    keep_idx <- target_hours_all <= max_lead
    target_hours_all  <- target_hours_all[keep_idx]
    target_labels_all <- target_labels_all[keep_idx]
    
    target_times <- t0 + target_hours_all * 3600
    
    step_to_k <- integer(length(target_hours_all))
    for (ii in seq_along(target_hours_all)) {
      diffs <- abs(lead_hours - target_hours_all[ii])
      step_to_k[ii] <- if (all(!is.finite(diffs))) NA_integer_ else which.min(diffs)
    }
    
    ice_fc_layers   <- vector("list", length(target_hours_all))
    climb_fc_layers <- vector("list", length(target_hours_all))
    names(ice_fc_layers)   <- target_labels_all
    names(climb_fc_layers) <- target_labels_all
    
    # Step 0: Analyse = aktuell
    if (length(target_hours_all) > 0 && target_hours_all[1] == 0) {
      ice_fc_layers[[1]]   <- ice_thick_expo
      climb_fc_layers[[1]] <- climb_r
    }
    
    FDH_base_r  <- r_FDH_eff
    delta_cum_r <- raster(FDH_base_r); delta_cum_r[] <- 0
    
    next_target_idx <- which(target_hours_all > 0)[1]
    if (is.na(next_target_idx)) next_target_idx <- length(target_hours_all) + 1
    
    # precompute solar_index raster for rad factor
    solar_index_r_use <- solar_index_r
    crs(solar_index_r_use) <- crs(r_template)
    extent(solar_index_r_use) <- extent(r_template)
    
    for (k in seq_along(time_fc)) {
      dt_k <- lead_hours[k]
      
      T_k  <- raster(T2M_fc,  layer = k)
      RH_k <- raster(RH2M_fc, layer = k) / 100
      W_k  <- raster(W_fc,    layer = k)
      
      FDH_k <- calc(T_k, fun = function(x) pmax(-x, 0))
      MDH_k <- calc(T_k, fun = function(x) pmax( x, 0))
      
      f_wind <- calc(W_k, fun = function(w) {
        wn <- (w - wind_ref) / wind_ref
        f  <- 1 + k_wind * wn
        f[!is.finite(f)] <- 1
        pmax(wind_min, pmin(wind_max, f))
      })
      
      f_rh <- calc(RH_k, fun = function(rh) {
        rh[is.na(rh)] <- rh_opt
        0.5 + 0.5 * exp(- (rh - rh_opt)^2 / (2 * rh_sig^2))
      })
      
      s_height_k <- solar_height_factor_fc[k]
      if (!is.finite(s_height_k)) s_height_k <- 0
      
      f_rad_k <- if (s_height_k == 0) {
        solar_index_r_use * 0 + 1
      } else {
        calc(solar_index_r_use, fun = function(s_index) {
          f <- 1 - k_expo * s_height_k * s_index
          f[!is.finite(f)] <- 1
          pmax(0, pmin(1, f))
        })
      }
      
      FDH_eff_k <- FDH_k * f_wind * f_rh * f_rad_k
      MDH_eff_k <- MDH_k * k_melt
      
      delta_cum_r <- delta_cum_r + (FDH_eff_k - MDH_eff_k)
      
      while (next_target_idx <= length(target_hours_all) &&
             dt_k >= target_hours_all[next_target_idx] - 0.5 &&
             is.null(ice_fc_layers[[next_target_idx]])) {
        
        FDH_fc_k <- FDH_base_r + delta_cum_r
        FDH_fc_k[FDH_fc_k < 0] <- 0
        
        ice_k <- FDH_fc_k * alpha
        ice_k[FDH_fc_k < FDH_crit] <- NA
        names(ice_k) <- paste0("h_pot_expo_m_fc_", target_hours_all[next_target_idx], "h")
        ice_fc_layers[[next_target_idx]] <- ice_k
        
        score_h_k  <- calc(ice_k, fun = function(h) score_h_fun(h, h_min, h_opt))
        score_T_k  <- calc(T_k,  fun = function(Tv) score_T_fun(Tv, T_opt, T_min, T_max, range_T))
        score_RH_k <- calc(RH_k, fun = function(rh) score_RH_fun(rh, RH_opt_c, RH_sig_c))
        
        climb_k <- overlay(score_h_k, score_T_k, score_T3_fc, score_RH_k,
                           fun = function(a, b, c, d) a * b * c * d)
        
        climb_k[is.na(ice_k) | ice_k <= h_min] <- NA
        climb_k[!is.finite(climb_k)]          <- NA
        climb_k[climb_k <= 0]                 <- NA
        names(climb_k) <- paste0("Climbability_fc_", target_hours_all[next_target_idx], "h")
        climb_fc_layers[[next_target_idx]] <- climb_k
        
        next_target_idx <- next_target_idx + 1
      }
    }
    
    # Fill nulls
    if (any(vapply(ice_fc_layers, is.null, logical(1)))) {
      last_non_null <- max(which(!vapply(ice_fc_layers, is.null, logical(1))))
      for (i in seq_along(ice_fc_layers)) {
        if (is.null(ice_fc_layers[[i]])) {
          ice_fc_layers[[i]]   <- ice_fc_layers[[last_non_null]]
          climb_fc_layers[[i]] <- climb_fc_layers[[last_non_null]]
        }
      }
    }
    
    fc_step_hours <- target_hours_all
  }
}

# ---------------- 8b) Kompakte Tagesübersicht (wie bei dir) -----------

best_html <- NULL

if (!is.null(climb_fc_layers) &&
    length(climb_fc_layers) > 0 &&
    !is.null(target_times) &&
    length(target_times) == length(climb_fc_layers) &&
    exists("time_fc") &&
    exists("T2M_fc") &&
    exists("RH2M_fc")) {
  
  times_local_all <- lubridate::with_tz(target_times, "Europe/Vienna")
  dates_all <- as.Date(times_local_all)
  
  # step -> nächster forecast index
  fc_idx_for_step <- integer(length(target_times))
  for (s in seq_along(target_times)) {
    dt <- abs(as.numeric(difftime(time_fc, target_times[s], units = "hours")))
    fc_idx_for_step[s] <- if (all(is.na(dt))) NA_integer_ else which.min(dt)
  }
  
  mean_all <- rep(NA_real_, length(climb_fc_layers))
  for (i in seq_along(climb_fc_layers)) {
    r_i <- climb_fc_layers[[i]]
    if (!is.null(r_i)) {
      mean_all[i] <- weighted_mean_alt(r_i, alt_weight_r)
    }
  }
  
  df_fc <- data.frame(
    step = seq_along(climb_fc_layers),
    time_local = times_local_all,
    date = dates_all,
    mean_ci = mean_all,
    fc_idx = fc_idx_for_step,
    stringsAsFactors = FALSE
  )
  df_fc <- df_fc[is.finite(df_fc$mean_ci), , drop = FALSE]
  
  if (nrow(df_fc) > 0) {
    
    hour_icon <- function(h) {
      if (h < 6 || h >= 22) "🌙" else if (h < 9) "🌅" else if (h < 18) "☀️" else "🌇"
    }
    
    temp_tag <- function(T_mean, best = TRUE) {
      if (!is.finite(T_mean)) return(NA_character_)
      if (T_mean <= -18) {
        if (best) "evtl. zu kalt / spröde" else "sehr kalt, spröde"
      } else if (T_mean <= -10) "sehr kalt"
      else if (T_mean <= -5)  if (best) "kalt, meist gut" else "kalt"
      else if (T_mean <= -2)  if (best) "Optimalbereich (-4…-3 °C)" else "nahe Optimum"
      else if (T_mean <= 0)   if (best) "mild, eher weich" else "mild"
      else "zu mild (>0 °C)"
    }
    
    rh_tag <- function(RH_mean) {
      if (!is.finite(RH_mean)) return(NA_character_)
      if (RH_mean < 0.30) "zu trocken"
      else if (RH_mean < 0.40) "eher trocken"
      else if (RH_mean <= 0.50) "opt. Feuchte (40–50 %)"
      else if (RH_mean <= 0.70) "leicht feucht"
      else "sehr feucht"
    }
    
    build_short_tag <- function(T_mean, RH_mean, best = TRUE) {
      tags <- na.omit(c(temp_tag(T_mean, best), rh_tag(RH_mean)))
      if (length(tags) == 0) return("")
      paste(tags, collapse = ", ")
    }
    
    pict_rows <- character(0)
    split_by_date <- split(df_fc, df_fc$date)
    
    for (d_str in names(split_by_date)) {
      block <- split_by_date[[d_str]]
      d <- as.Date(d_str)
      
      n_block <- nrow(block)
      T_means_day  <- rep(NA_real_, n_block)
      RH_means_day <- rep(NA_real_, n_block)
      
      for (j in seq_len(n_block)) {
        k <- block$fc_idx[j]
        if (is.finite(k) && k >= 1 && k <= raster::nlayers(T2M_fc)) {
          T_r_j  <- raster::raster(T2M_fc, layer = k)
          RH_r_j <- raster::raster(RH2M_fc, layer = k) / 100
          T_means_day[j]  <- weighted_mean_alt(T_r_j, alt_weight_r)
          RH_means_day[j] <- weighted_mean_alt(RH_r_j, alt_weight_r)
        }
      }
      
      mask_good <- is.finite(T_means_day) & (T_means_day >= -18) & (T_means_day <= 0)
      if (any(mask_good)) {
        ci_cand <- block$mean_ci
        ci_cand[!mask_good] <- NA
        idx_best_local <- which.max(ci_cand)
      } else {
        idx_best_local <- which.max(block$mean_ci)
      }
      
      idx_min_local <- which.min(block$mean_ci)
      
      row_best <- block[idx_best_local, ]
      row_min  <- block[idx_min_local, ]
      
      t_best <- row_best$time_local
      t_min  <- row_min$time_local
      
      h_best <- lubridate::hour(t_best) + lubridate::minute(t_best) / 60
      h_min  <- lubridate::hour(t_min)  + lubridate::minute(t_min)  / 60
      
      icon_best <- hour_icon(h_best)
      icon_min  <- hour_icon(h_min)
      
      T_mean_best  <- T_means_day[idx_best_local]
      RH_mean_best <- RH_means_day[idx_best_local]
      T_mean_min   <- T_means_day[idx_min_local]
      RH_mean_min  <- RH_means_day[idx_min_local]
      
      tag_best <- build_short_tag(T_mean_best, RH_mean_best, best = TRUE)
      tag_min  <- build_short_tag(T_mean_min,  RH_mean_min,  best = FALSE)
      
      pict_rows <- c(
        pict_rows,
        sprintf(
          "<tr>
             <td>%s</td>
             <td>
               ▲ %s&nbsp;%s<br/>
               <span style='font-size:10px;'>%s</span>
             </td>
             <td>
               ▼ %s&nbsp;%s<br/>
               <span style='font-size:10px;'>%s</span>
             </td>
           </tr>",
          format(d, "%d.%m."),
          format(t_best, "%H:%M"), icon_best, tag_best,
          format(t_min,  "%H:%M"), icon_min,  tag_min
        )
      )
    }
    
    if (length(pict_rows) > 0) {
      table_html <- paste0(
        "<table style='width:100%; border-collapse:collapse; font-size:12px;'>",
        "<tr><th style='text-align:left;'>Tag</th><th style='text-align:left;'>Beste Zeit</th><th style='text-align:left;'>Schlechteste Zeit</th></tr>",
        paste(pict_rows, collapse = ""),
        "</table>"
      )
      
      best_html <- htmltools::HTML(
        paste0(
          "<div id='climb-summary-box' style='font-size: 13px; background: rgba(255,255,255,0.9);",
          "padding: 6px 8px; border-radius: 6px; max-width: 420px; line-height: 1.4;'>",
          "<div id='climb-summary-header' style='font-weight:bold; cursor:pointer; margin-bottom:4px;'>",
          "Kletterbarkeit – Tagesübersicht (Prognose) ▾</div>",
          "<div id='climb-summary-body' style='display:none;'>",
          table_html,
          "<div style='font-size:11px; margin-top:4px;'>",
          "▲ höchster, ▼ niedrigster mittlerer Climbability-Wert (Gebietsmittel, höhengewichtet).<br/>",
          "<span style='font-size:10px;'>\"evtl. zu kalt\" bedeutet: z.B. &lt; −18 °C, Eis kann spröde sein.</span>",
          "</div></div></div>"
        )
      )
    }
  }
}

# ------------------ 9) Leaflet: Zeit-Layer zusammensetzen ------------

ice_time_layers   <- list()
climb_time_layers <- list()
time_labels       <- character(0)

if (length(ice_hist_layers) > 0L) {
  ice_time_layers   <- c(ice_time_layers, ice_hist_layers)
  climb_time_layers <- c(climb_time_layers, climb_hist_layers)
  time_labels       <- c(time_labels, ice_hist_labels)
}

if (!is.null(ice_fc_layers) && length(ice_fc_layers) > 0L) {
  ice_time_layers   <- c(ice_time_layers, ice_fc_layers)
  climb_time_layers <- c(climb_time_layers, climb_fc_layers)
  time_labels       <- c(time_labels, names(ice_fc_layers))
}

if (length(ice_time_layers) == 0L) {
  ice_time_layers   <- list(ice_thick_expo)
  climb_time_layers <- list(climb_r)
  time_labels       <- format(Sys.Date(), "%d.%m.%Y (Jetzt)")
}

n_steps <- min(length(ice_time_layers), length(climb_time_layers), length(time_labels))
ice_time_layers   <- ice_time_layers[seq_len(n_steps)]
climb_time_layers <- climb_time_layers[seq_len(n_steps)]
time_labels       <- time_labels[seq_len(n_steps)]

max_h_all <- vapply(
  ice_time_layers,
  function(r) if (is.null(r)) NA_real_ else suppressWarnings(cellStats(r, "max", na.rm = TRUE)),
  numeric(1)
)
max_h <- max(max_h_all, na.rm = TRUE)
if (!is.finite(max_h) || max_h <= 0) max_h <- 1

col_fun <- colorRampPalette(c("#ffffff", "#c6dbef", "#6baed6", "#08519c"))

pal_h <- colorNumeric(palette = col_fun(100), domain = c(0, max_h), na.color = "transparent")
pal_ci <- colorNumeric(palette = rev(terrain.colors(100)), domain = c(0, 1), na.color = "transparent")

last_update <- format(Sys.time(), "%d.%m.%Y %H:%M UTC")
ext <- extent(r_template)

# ------------------ 9a) Eisfall-Sonnendaten / Topo -------------------

sun_df <- readr::read_csv(
  "data/Koordinaten_Wasserfaelle/icefalls_sun_horizon_2024_2025.csv",
  show_col_types = FALSE
)

if (!inherits(sun_df$sunrise_topo, "POSIXt")) {
  sun_df <- sun_df %>%
    mutate(
      sunrise_topo = ymd_hms(sunrise_topo, tz = "UTC"),
      sunset_topo  = ymd_hms(sunset_topo,  tz = "UTC")
    )
}

sun_df <- sun_df %>%
  mutate(
    sunrise_topo = with_tz(sunrise_topo, "Europe/Vienna"),
    sunset_topo  = with_tz(sunset_topo,  "Europe/Vienna"),
    sun_hours_topo = as.numeric(difftime(sunset_topo, sunrise_topo, units = "hours"))
  )

sun_date <- Sys.Date()
sun_today <- sun_df %>% filter(date == sun_date)

if (nrow(sun_today) == 0) {
  last_date <- max(sun_df$date, na.rm = TRUE)
  message("Keine Sonnendaten für ", sun_date, " gefunden. Verwende stattdessen ", last_date, ".")
  sun_today <- sun_df %>% filter(date == last_date)
}

if (nrow(sun_today) == 0) {
  stop("sun_today ist leer – prüfe icefalls_sun_horizon_2024_2025.csv (Spaltennamen / date-Typ).")
}

sun_today <- sun_today %>%
  mutate(
    sunrise_txt   = substr(as.character(sunrise_topo), 12, 16),
    sunset_txt    = substr(as.character(sunset_topo),  12, 16),
    sun_hours_txt = sprintf("%.1f", sun_hours_topo),
    date_txt      = format(date, "%d.%m.%Y"),
    link_txt = ifelse(
      !is.na(topo_url) & topo_url != "",
      paste0("<a href='", topo_url, "' target='_blank'>Topo öffnen</a>"),
      "(kein Topo-Link hinterlegt)"
    ),
    popup = ifelse(
      is.na(sun_hours_topo) | is.na(sunrise_topo) | is.na(sunset_topo),
      sprintf("<b>%s</b><br/>Sonne am %s: keine direkte Sonneneinstrahlung<br/>%s",
              name, date_txt, link_txt),
      sprintf("<b>%s</b><br/>Sonne am %s: %s – %s (%s h)<br/>%s",
              name, date_txt, sunrise_txt, sunset_txt, sun_hours_txt, link_txt)
    )
  )

# ------------------ 9b) Leaflet bauen (projiziert) --------------------

m <- leaflet() |>
  addProviderTiles(providers$OpenStreetMap, group = "OSM") |>
  addProviderTiles(providers$OpenTopoMap,   group = "Gelände (Topo)")

if (!is.null(best_html)) {
  m <- m |> addControl(position = "bottomright", html = best_html)
}

# Raster-Layer: robust mit projectRasterForLeaflet + project=FALSE
if (length(ice_time_layers) > 0L) {
  visible_step_ice <- length(ice_time_layers)
  for (i in seq_along(ice_time_layers)) {
    r_i <- ice_time_layers[[i]]
    if (!is.null(r_i)) {
      m <- leaflet_add_projected_raster(
        map = m,
        r = r_i,
        pal = pal_h,
        opacity = if (i == visible_step_ice) 0.8 else 0.0,
        layer_id = paste0("ice_step_", i),
        group = "Eisdicke"
      )
    }
  }
}

if (length(climb_time_layers) > 0L) {
  visible_step_climb <- length(climb_time_layers)
  for (i in seq_along(climb_time_layers)) {
    r_i <- climb_time_layers[[i]]
    if (!is.null(r_i)) {
      m <- leaflet_add_projected_raster(
        map = m,
        r = r_i,
        pal = pal_ci,
        opacity = if (i == visible_step_climb) 0.7 else 0.0,
        layer_id = paste0("climb_step_", i),
        group = "Climbability"
      )
    }
  }
}

m <- m |>
  fitBounds(lng1 = ext@xmin, lat1 = ext@ymin, lng2 = ext@xmax, lat2 = ext@ymax) |>
  addLegend(pal = pal_ci, values = c(0, 1), title = "Climbability",
            labFormat = labelFormat(digits = 1), position = "bottomleft") |>
  addLegend(pal = pal_h, values = c(0, max_h), title = "Eisdicke (m)",
            labFormat = labelFormat(digits = 2), position = "bottomleft") |>
  addCircleMarkers(
    data = sun_today,
    lng = ~longitude, lat = ~latitude,
    radius = 5, color = "black", weight = 1,
    fillColor = "orange", fillOpacity = 0.9,
    popup = ~popup, group = "Eisfälle"
  ) |>
  addLayersControl(
    baseGroups = c("OSM", "Gelände (Topo)"),
    overlayGroups = c("Eisdicke", "Climbability", "Eisfälle"),
    options = layersControlOptions(collapsed = FALSE)
  ) |>
  addControl(
    position = "bottomright",
    html = htmltools::HTML(
      paste0(
        "<div id='impressum-box' style='font-size: 14px; background: rgba(255,255,255,0.9);",
        "padding: 6px 8px; border-radius: 6px; max-width: 340px; line-height: 1.4;'>",
        "<div id='impressum-header' style='font-weight:bold; cursor:pointer; margin-bottom:4px;'>",
        "Impressum / Quellen ▾</div>",
        "<div id='impressum-body' style='display:none;'>",
        "<strong>Quellen</strong><br/>",
        "INCA (GeoSphere Austria, <a href='https://doi.org/10.60669/6akt-5p05' target='_blank'>doi:10.60669/6akt-5p05</a>)<br/>",
        "NWP AROME (GeoSphere Austria, <a href='https://doi.org/10.60669/9zm8-s664' target='_blank'>doi:10.60669/9zm8-s664</a>)<br/>",
        "DEM Tirol (<a href='https://www.data.gv.at/katalog/datasets/0454f5f3-1d8c-464e-847d-541901eb021a' target='_blank'>data.gv.at</a>)<br/><br/>",
        "<em>Letztes Update: ", last_update, "</em>",
        "</div></div>"
      )
    )
  )

# Collapsible controls JS
m <- htmlwidgets::onRender(
  m,
  "
  function(el, x) {
    function makeToggle(headerId, bodyId, downTxt, upTxt) {
      var header = el.querySelector(headerId);
      var body   = el.querySelector(bodyId);
      if (!header || !body) return;
      body.style.display = 'none';
      header.addEventListener('click', function() {
        var visible = body.style.display !== 'none';
        body.style.display = visible ? 'none' : 'block';
        header.innerHTML = visible ? downTxt : upTxt;
      });
    }
    makeToggle('#impressum-header', '#impressum-body', 'Impressum / Quellen ▾', 'Impressum / Quellen ▴');
    makeToggle('#climb-summary-header', '#climb-summary-body', 'Kletterbarkeit – Tagesübersicht (Prognose) ▾', 'Kletterbarkeit – Tagesübersicht (Prognose) ▴');
  }
  "
)

# Zeit-Slider JS (wie bei dir, aber bleibt kompatibel, auch wenn Raster projiziert wurden)
if (length(time_labels) > 0L) {
  labels_js  <- paste0("['", paste(time_labels, collapse = "','"), "']")
  n_steps_js <- n_steps
  
  js_code <- sprintf(
    "
    function(el, x) {
      var map = this;

      var lc = el.getElementsByClassName('leaflet-control-layers-expanded')[0]
            || el.getElementsByClassName('leaflet-control-layers')[0];
      if (lc) {
        lc.style.marginTop   = '10px';
        lc.style.marginRight = '90px';
        lc.style.transform   = 'scale(1.5)';
        lc.style.transformOrigin = 'top left';
        lc.style.padding     = '12px 15px';
        lc.style.fontSize    = '16px';
      }

      var labels = %s;
      var nSteps = %d;

      if (!labels || labels.length === 0 || nSteps <= 0) return;

      var initial = nSteps - 1;
      if (initial < 0) initial = 0;

      function splitLayersByOrder() {
        var rasters = [];
        map.eachLayer(function(layer) {
          if (!layer || !layer.options) return;
          if (layer instanceof L.TileLayer) return;
          if (typeof layer.setOpacity !== 'function') return;
          rasters.push(layer);
        });
        var iceLayers = [], climbLayers = [];
        if (rasters.length >= 2 * nSteps) {
          for (var i = 0; i < nSteps; i++) iceLayers.push(rasters[i]);
          for (var j = 0; j < nSteps; j++) climbLayers.push(rasters[nSteps + j]);
        } else {
          var half = Math.floor(rasters.length / 2);
          for (var i = 0; i < half; i++) iceLayers.push(rasters[i]);
          for (var j = half; j < rasters.length; j++) climbLayers.push(rasters[j]);
        }
        return { ice: iceLayers, climb: climbLayers };
      }

      var layerCache = splitLayersByOrder();

      function setTimeStep(step) {
        if (step < 0) step = 0;
        if (step >= nSteps) step = nSteps - 1;

        (layerCache.ice || []).forEach(function(layer, idx) {
          if (layer && typeof layer.setOpacity === 'function') layer.setOpacity(idx === step ? 0.8 : 0.0);
        });
        (layerCache.climb || []).forEach(function(layer, idx) {
          if (layer && typeof layer.setOpacity === 'function') layer.setOpacity(idx === step ? 0.7 : 0.0);
        });

        var labelDiv = document.getElementById('time-label');
        if (labelDiv && step >= 0 && step < labels.length) labelDiv.textContent = labels[step];

        var slider = document.getElementById('time-slider');
        if (slider && slider.value !== String(step)) slider.value = step;
      }

      var sliderControl = L.control({position: 'topright'});
      sliderControl.onAdd = function() {
        var div = L.DomUtil.create('div', 'info leaflet-control');
        div.style.background   = 'rgba(255,255,255,0.9)';
        div.style.padding      = '8px 10px';
        div.style.borderRadius = '6px';
        div.style.minWidth     = '260px';
        div.style.marginTop    = '70px';
        div.style.marginRight  = '10px';

        var title = document.createElement('div');
        title.style.fontSize = '16px';
        title.style.marginBottom = '4px';
        title.innerHTML = '<b>Eisdicke & Climbability – Zeitverlauf</b>';
        div.appendChild(title);

        var labelDiv = document.createElement('div');
        labelDiv.id = 'time-label';
        labelDiv.style.fontSize = '14px';
        labelDiv.style.marginBottom = '4px';
        labelDiv.textContent = labels[initial] || labels[0];
        div.appendChild(labelDiv);

        var slider = document.createElement('input');
        slider.type = 'range';
        slider.min = 0;
        slider.max = nSteps - 1;
        slider.step = 1;
        slider.value = initial;
        slider.style.width = '240px';
        slider.id = 'time-slider';
        div.appendChild(slider);

        slider.addEventListener('input', function(e) {
          var step = parseInt(e.target.value, 10);
          if (!isNaN(step)) setTimeStep(step);
        });

        slider.addEventListener('mousedown', function(e) { if (map && map.dragging) map.dragging.disable(); });
        slider.addEventListener('mouseup', function(e) { if (map && map.dragging) map.dragging.enable(); });

        L.DomEvent.disableClickPropagation(div);
        return div;
      };
      sliderControl.addTo(map);

      setTimeout(function() { setTimeStep(initial); }, 500);
    }
    ",
    labels_js,
    n_steps_js
  )
  
  m <- htmlwidgets::onRender(m, js_code)
}

saveWidget(m, "eisdicke_nordtirol.html", selfcontained = TRUE)
